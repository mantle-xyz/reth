...
pub struct OpPoolBuilder<T = crate::txpool::OpPooledTransaction> {
    ...
    /// Interop filter URLs for txpool-level interop validation. When empty, interop transaction
...
impl<T> OpPoolBuilder<T> {
    ...
    /// Sets the interop filter URLs. Pass an empty vec to disable interop transaction validation.
    pub fn with_interop(
        mut self,
        interop_client: Vec<String>,
    ...
    Evm: ConfigureEvm<Primitives = PrimitivesTy<Node::Types>> + Clone + 'static,
{
    ...
    async fn build_pool(
        ...
        let interop_client = if !self.interop_http.is_empty() {
            Some(
                InteropFilterClient::builder(self.interop_http.clone(), ctx.chain_spec().chain_id())
