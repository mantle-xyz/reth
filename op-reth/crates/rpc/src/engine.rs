...
pub trait OpEngineApi<Engine: EngineTypes> {
    ...
    ///
    /// OP modifications:
    /// - the response type is [`EngineTypes::ExecutionPayloadEnvelopeV5`], which on OP/Mantle L2
    ///   is identical to V4 (L2 blocks carry no blobs, so the blobs-bundle change is a no-op).
    ///   Required by op-node once the Mantle Limb hardfork is active.
