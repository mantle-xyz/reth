...
//! verify:
//!
//! - `preconf_tx_over_da_limit_rejected_and_not_on_chain` — a large-calldata preconf tx (estimated
//!   DA well over the limit) is rejected by the gate; the client sees an error and the tx never
//!   lands.
//! - `preconf_tx_within_da_limit_lands` — a plain transfer (estimated DA at the 100-byte floor)
//!   under the same config lands normally, guarding against the gate over-rejecting.
//!
