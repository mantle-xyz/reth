//! Axum websocket server accepting downstream subscribers.
...
impl Server {
    ...
    /// Binds the listening socket without serving on it.
    ///
    /// Split out from [`Self::listen`] so a caller that asked for port 0 can
    /// learn the port that was actually assigned before any client connects.
    ...
    pub async fn listen(&self, cancellation_token: CancellationToken) {
        ...
        self.serve(listener, cancellation_token).await.unwrap()
    }

    /// Serves on an already-bound listener until the cancellation token fires.
    pub async fn serve(
        &self,
        ...
        info!(message = "starting server", address = ?listener.local_addr());
