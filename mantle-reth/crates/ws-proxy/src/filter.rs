//! Address and event-topic filtering of flashblock payloads.
//!
//! Reads `metadata.receipts[*].logs` from the slice JSON, so it depends on the
//! producer populating that map. Decompresses brotli first when the upstream
...
impl FilterType {
    ...
    /// Every log in `metadata.receipts`, across both receipt encodings.
    ///
    /// `OpReceipt` serialises flat — `{"type", "status", "cumulativeGasUsed",
    /// "logs"}` — so `logs` sits directly on the receipt. Base's `BaseReceipt`
    /// is an externally tagged enum, putting it one level down under the
    /// variant name (`{"Deposit": {"logs": ...}}`). Both upstreams are accepted:
    /// reading only the nested form would make every filter silently match
    /// nothing against a Mantle payload.
    fn receipt_logs(json: &Value) -> impl Iterator<Item = &Value> {
        json.get("metadata")
            ...
            .flat_map(|receipt| {
                receipt
                    ...
                    .map(|logs| logs.iter().collect::<Vec<_>>())
                    .unwrap_or_else(|| {
                        receipt
                            .as_object()
                            .into_iter()
                            ...
                            .filter_map(|variant| {
                                variant.get("logs").and_then(|logs| logs.as_array())
                            })
                            .flatten()
                            .collect()
                    })
    ...
    fn contains_any_address(&self, json: &Value, addresses: &HashSet<String>) -> bool {
        ...
        for log in Self::receipt_logs(json) {
            if let Some(addr_str) = log.get("address").and_then(|addr| addr.as_str()) &&
    ...
    fn contains_any_topic(&self, json: &Value, topics: &HashSet<String>) -> bool {
        ...
        for log in Self::receipt_logs(json) {
            if let Some(log_topics) = log.get("topics").and_then(|topics| topics.as_array()) {
...
/// Filtering against a payload built from the real Mantle wire types.
///
/// The fixtures in [`tests`] above are Base-shaped JSON, kept because a
/// Base-derived upstream (rollup-boost) may still emit them. These lock the
/// shape this proxy will actually see from a Mantle producer: `OpReceipt`
/// serialises flat, so `logs` sits directly on the receipt rather than one
/// level down under a variant name.
///
/// **For the producer side**: these pass only while `metadata.receipts` is
/// populated. It is `BTreeMap::default()` today, which makes every
/// address/topic filter match nothing — see the plan discussion issue on
/// `metadata.receipts`.
#[cfg(test)]
mod mantle_shape {
    ...
    const WATCHED: Address = Address::repeat_byte(0x42);
    const OTHER: Address = Address::repeat_byte(0x99);
    const TOPIC: B256 = B256::repeat_byte(0xb0);

    fn receipt(address: Address, topic: B256) -> OpReceipt {
    ...
    /// A slice carrying one receipt, serialised exactly as the producer sends it.
    fn payload(address: Address, topic: B256) -> Vec<u8> {
        let mut metadata = MantleFlashblockMetadata::default();
        metadata.receipts.insert(B256::repeat_byte(0x11), receipt(address, topic));

        let slice = MantleFlashblockPayload { metadata, ..MantleFlashblockPayload::default() };
        serde_json::to_vec(&slice).expect("the wire format serialises")
    }

    #[test]
    fn an_address_filter_matches_a_flat_op_receipt() {
        let filter = FilterType::new_addresses(vec![format!("{WATCHED:#x}")]);

        assert!(filter.matches(&payload(WATCHED, TOPIC), false), "the watched address must match");
        assert!(!filter.matches(&payload(OTHER, TOPIC), false), "an unrelated address must not");
    }

    #[test]
    fn a_topic_filter_matches_a_flat_op_receipt() {
        let filter = FilterType::new_topics(vec![format!("{TOPIC:#x}")]);

        assert!(filter.matches(&payload(WATCHED, TOPIC), false), "the watched topic must match");
        assert!(
            !filter.matches(&payload(WATCHED, B256::repeat_byte(0x01)), false),
            "an unrelated topic must not"
        );
    }

    /// `All` requires both sides, so a payload matching only the address is a
    /// miss. Guards against the two predicates being OR-ed by accident.
    #[test]
    fn a_combined_filter_honours_match_mode() {
        let both = FilterType::new_combined_with_mode(
            vec![format!("{WATCHED:#x}")],
            vec![format!("{TOPIC:#x}")],
            MatchMode::All,
        );
        assert!(both.matches(&payload(WATCHED, TOPIC), false));
        assert!(!both.matches(&payload(WATCHED, B256::repeat_byte(0x01)), false));

        let either = FilterType::new_combined_with_mode(
            vec![format!("{WATCHED:#x}")],
            vec![format!("{TOPIC:#x}")],
            MatchMode::Any,
        );
        assert!(either.matches(&payload(WATCHED, B256::repeat_byte(0x01)), false));
    }

    /// The state the producer ships today: the map is present but empty, so
    /// there are no logs to match. Locks the fact that this is a miss rather
    /// than a match-everything, and will start failing once the producer
    /// populates the map — which is the intended signal.
    #[test]
    fn an_empty_receipts_map_matches_nothing() {
        let slice = MantleFlashblockPayload::default();
        let bytes = serde_json::to_vec(&slice).expect("the wire format serialises");

        let filter = FilterType::new_addresses(vec![format!("{WATCHED:#x}")]);
        assert!(!filter.matches(&bytes, false));

        assert!(
            FilterType::None.matches(&bytes, false),
            "an absent filter still passes everything"
        );
    }
}
