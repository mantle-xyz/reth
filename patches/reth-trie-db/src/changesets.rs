...
/// [MANTLE PATCH] Hard upper bound on the span of block numbers retained in the changeset cache.
///
/// Upstream reth relies solely on the external `evict(threshold)` call (from the engine tree's
/// `on_persistence_complete`), whose threshold is `min(finalized, last_persisted - 64)`. On an L2
/// follower whose finalized block stays at genesis, that threshold pins at ~0 and the cache never
/// evicts, so it accumulates an `Arc<TrieUpdatesSorted>` for every block from genesis and grows
/// without bound (observed ~467k entries during a from-genesis sync → multi-GB anonymous-heap leak).
///
/// This cap bounds retention to the most recent `MAX_CHANGESET_CACHE_BLOCKS` block numbers,
/// independent of the external eviction cadence. It is chosen well above any healthy unpersisted
/// in-memory window, so only changesets for long-persisted blocks — recomputable from the DB on a
/// cache miss — are ever dropped.
const MAX_CHANGESET_CACHE_BLOCKS: u64 = 1024;
...
impl ChangesetCacheInner {
    ...
    fn insert(&mut self, block_hash: B256, block_number: u64, changesets: Arc<TrieUpdatesSorted>) {
        ...
        // [MANTLE PATCH] Enforce a hard upper bound on the retained block range so the cache
        // stays bounded even when the external `evict(threshold)` is pinned at ~0 (follower with
        // finalized stuck at genesis). Evict everything below `max_block - MAX_CHANGESET_CACHE_BLOCKS`.
        let cap_floor = self
            .block_numbers
            .keys()
            .next_back()
            .map(|&max_block| max_block.saturating_sub(MAX_CHANGESET_CACHE_BLOCKS));
        if let Some(floor) = cap_floor {
            if floor > 0 {
                self.evict(floor);
            }
        }
