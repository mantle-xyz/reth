    ...
    fn on_persistence_complete(
        ...
        let eviction_threshold =
            if let Some(finalized) = self.canonical_in_memory_state.get_finalized_num_hash() {
                // [MANTLE PATCH] Use `max` (not `min`) of finalized and the retention threshold.
                // Upstream's `min` keeps changesets back to `finalized`; on an L2 follower whose
                // finalized block stays at genesis, that pins the threshold at ~0 → the changeset
                // cache never evicts and grows without bound (observed ~467k entries, multi-GB
                // anon leak). `max` always evicts below `last_persisted - 64`, so the cache is
                // bounded to the unpersisted window + retention, while still never evicting an
                // unpersisted in-memory block's changeset (threshold >= last_persisted - 64).
                finalized.number.max(min_threshold)
            } else {
